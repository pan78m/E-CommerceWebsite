...
if (isset($_SESSION["uid"])) {
    ...
    // Validate if the zip code is provided
       if (empty($zip)) {
        echo "<script>alert('Please provide a zip code.'); window.location.href='checkout.php';</script>";
        exit;
    }
    // Validate the length of the zip code
    // if (strlen($zip) > 10 || strlen($zip) < 4) { // Adjust the length according to your requirements
    //     echo "<script>alert('Invalid zip code length.'); window.location.href='checkout.php';</script>";
    //     exit;
    // }
